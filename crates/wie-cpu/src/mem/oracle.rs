//! Property-style oracle: mmap_page vs mmap_arena must agree (storage parity).
//!
//! Time-budgeted for CI: fixed PRNG seed, capped operation count.
//! After the great cleanup, HashMap is gone — both sides are soft-translated mmap.
...
fn run_oracle_arena_vs_page(seed: u64, n_ops: usize) {
    run_oracle_pair(
        seed,
        n_ops,
        "mmap_arena vs mmap_page",
...
#[test]
fn oracle_mmap_arena_vs_page_default_seed() {
    // ~2k ops keeps CI under a second on Apple Silicon.
    run_oracle_arena_vs_page(0x001E_BEEF_u64, 2_000);
}

#[test]
fn oracle_mmap_arena_vs_page_alt_seeds() {
    for seed in [1_u64, 42, 0xDEAD_BEEF, 0x00C0_FFEE] {
        run_oracle_arena_vs_page(seed, 500);
    }
}

#[test]
fn oracle_page_ptr_walk_agrees() {
    let mut a = MmapArenaBackend::new();
    let mut b = MmapPageBackend::new();
    a.map(0x20_0000, 0x2000, 7).expect("mmap_arena map");
    b.map(0x20_0000, 0x2000, 7).expect("mmap_page map");
    let payload = b"hello-oracle";
    a.write(0x20_0040, payload).expect("arena write");
    b.write(0x20_0040, payload).expect("page write");
    let mut ha = [0_u8; 12];
    let mut hb = [0_u8; 12];
    a.read(0x20_0040, &mut ha).expect("arena read");
    b.read(0x20_0040, &mut hb).expect("page read");
    assert_eq!(ha, hb);
    ...
    let p0 = a.page_data_ptr_walk(0x20_0000 >> 12).expect("a0");
    let p1 = a.page_data_ptr_walk(0x20_1000 >> 12).expect("a1");
